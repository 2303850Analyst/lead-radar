...
export default function LeadRadarApp() {
  ...
  const [relevanceFilter, setRelevanceFilter] = useState("recommended");
  ...
  const filteredLeads = useMemo(() => {
    const leads = (response?.leads ?? []).filter((lead) => {
      ...
      const leadRelevance = relevanceStatus(lead);
      if (
        relevanceFilter === "recommended"
          ? !["matched", "maybe"].includes(leadRelevance)
          : relevanceFilter !== "any" && leadRelevance !== relevanceFilter
      ) return false;
