...
- 2GIS Places больше не обрезается первой страницей: адаптер учитывает
  provider `total` и последовательно получает страницы до конца доступной
  выдачи либо настроенного серверного предела, объединяя и дедуплицируя
  карточки между страницами и поисковыми формулировками. Demo-профиль забирает
  все разрешённые пять страниц по 10 результатов; production использует по 50
  результатов и по умолчанию до 20 страниц на arm, а `DGIS_MAX_PAGES` позволяет
  настроить глубину в пределах 1..100. Health API теперь сообщает размер
  страницы, глубину и максимальный объём одного arm; raw-ответы не сохраняются.

- Исправлен молчаливый останов после зелёной трактовки локализованного
  open-world запроса в режиме 2GIS. Общий admission больше не применяет к
  deterministic fallback Kimi-only требование английского retrieval term:
  schema-valid однозначный физический intent с русскими формулировками доходит
  до свободнотекстового поиска 2GIS. Дополнительно UI больше не скрывает
  `SEARCH_PLAN_UNSUPPORTED`, если приложенный сервером план сам имеет статус
  `ready`; противоречивый terminal outcome показывается как ошибка, а не как
  визуальное «зависание».

- Завершена provider-aware миграция вспомогательной географии 2GIS: при
  `SEARCH_PROVIDER=2gis` `POST /api/geocode` и серверное определение центра
  обращаются к 2GIS Geocoder, каталог/поиск метро — к 2GIS Places с
  `type=station.metro`, а выбранная станция повторно сверяется через 2GIS
  `/items/byid`. Health и UI показывают фактический источник, ссылка карточки
  открывает соответствующий объект на 2GIS. Явный режим Geoapify сохраняет
  прежние endpoints и атрибуцию; автоматического смешивания источников нет.

- Права 2GIS разделены на независимые capability вместо одного грубого
  `contract_required`: MapGL-карта разрешена для 2GIS-выдачи, CSV включается
  только явным серверным `DGIS_EXPORT_ENABLED=true`, а localStorage остаётся
  выключенным. Экспорт содержит нормализованные строки и обязательную
  атрибуцию; raw upstream response по-прежнему не сохраняется. Это устраняет
  ошибочный запрет карты и позволяет договорной лицензии разрешить выгрузку,
  не открывая лишние способы хранения.

- Обе встроенные Leaflet/OpenStreetMap-карты заменены на 2GIS MapGL: выбор
  области сохраняет клик по карте, перетаскивание и клавиатурное перемещение
  центральной метки, радиус и подгонку viewport; карта выдачи сохраняет
  кликабельные маркеры, выделение лида, подсказки и масштабирование по найденным
  координатам или исходному радиусу. Добавлен `DGIS_MAP_KEY` для отдельного
  browser-visible ключа Map Tiles; локально при его отсутствии используется
  уже настроенный `DGIS_API_KEY`. Этот fallback разрешён только для локальной
  проверки, поскольку ключ неизбежно попадает в browser payload. Leaflet и
  прямое обращение к публичному OSM tile endpoint удалены.

- Добавлен выбираемый серверный источник `2gis`: при
  `SEARCH_PROVIDER=2gis` и настроенном `DGIS_API_KEY` однозначный физический
  бизнес доходит до 2GIS Places API как свободный текст без ручного alias или
  обязательного `rubric_id`. Адаптер выполняет до трёх bounded arms по
  provider-neutral типу и синонимам, передаёт центр и радиус не более 50 км,
  ограничивает demo-ответ десятью карточками на запрос, дедуплицирует и
  нормализует карточки в общий контракт LeadRadar. Введённый адрес
  геокодируется 2GIS Geocoder; автоматического смешивания выдач двух источников
  нет. Пустой валидный ответ 2GIS становится `success_empty`, а
  не unsupported-ошибкой. Если поздняя текстовая формулировка падает после уже
  успешной, найденные карточки сохраняются, а `degradedStages` и notice явно
  отмечают частичную деградацию.
- Demo-ответы 2GIS получают policy `contract_required`: raw upstream response
  не сохраняется, интерфейс показывает атрибуцию 2GIS и блокирует localStorage,
  сохранение задания и CSV-экспорт. `DGIS_CONTACTS_ENABLED=false` фиксирует
  отсутствие отдельно подтверждённых прав на контактные поля; сам API-ключ и
  личный исследовательский сценарий не снимают лицензионное ограничение.
  Ключ остаётся server-only без `NEXT_PUBLIC_` и не включается в health, логи
  или клиентский payload.

- Свободный Geoapify Forward Geocoding fallback больше не принимает любую
  именованную организацию рядом с выбранным центром. Карточка из широкого
  текстового поиска попадает в выдачу только при локально подтверждённом
  `matched`/`maybe` evidence исходного типа бизнеса; нерелевантные соседи вроде
  банкомата, ресторана или офиса отбрасываются, а их отсутствие даёт честный
  `success_empty`. Пользовательские счётчики теперь отражают принятые после
  проверки карточки, а не сырой ответ geocoder. Ручных aliases не добавлено;
  compiler/runtime policy повышена до
  `semantic-retrieval-v2/2026-08-22.2`.

- Новый поисковый экран больше не подставляет демонстрационный запрос про
  фулфилмент, смежные складские формулировки и исключения. Поля целевого
  бизнеса, основного запроса, смежных запросов и исключений открываются пустыми;
  география и настройки предложения сохранены как отдельные рабочие defaults.

- Обязательная геокодировка введённого адреса теперь получает до 15 секунд
  внутри общего 60-секундного search deadline вместо прежних 5 секунд. Лимит
  согласован с сетевым timeout Geoapify: медленный ответ определения центра
  больше не обрывает поиск раньше остальных provider-этапов; уже выбранные на
  карте координаты по-прежнему пропускают этот запрос полностью.
