...
- Принята архитектура `Semantic Query Planner`: свободный запрос пользователя
  сначала переводится в независимое canonical business concept и
  версионированный `SearchPlan`, а только затем детерминированно компилируется в
  allowlisted категории Geoapify. Причина: восемь regex-правил внутри provider
  не покрывают произвольные формулировки и смешивают бизнес-смысл с контрактом
  одного источника.
- Kimi определён как ограниченный semantic resolver, но не как источник
  организаций, контактов или provider parameters. Модель не сможет формировать
  URL или category IDs: она выбирает только из переданного allowlisted каталога,
  а ответ проходит strict JSON Schema, MFJS и локальную allowlist. При слабом
  lexical match ей передаётся весь компактный каталог, чтобы фразы без общих
  токенов не получали ложный `unsupported`.
- Post-search Kimi classifier вынесен под независимый выключенный по умолчанию
  флаг. Реальные карточки нельзя передавать модели до документированной проверки
  условий источника, применимого privacy/data-flow и режима Customer Content;
  базовый релеванс в `v0.4.0` остаётся детерминированным.
- Для подтверждения неоднозначной трактовки принят подписанный token с TTL, а
  идентичность запроса (`requestCacheKey`) отделена от immutable результата
  планирования (`planHash`). Это не позволяет подтвердить устаревший или не
  предложенный concept.
- Зафиксирована строгая граница provider adapter: location-only запрос сначала
  геокодируется, затем server compiler создаёт execution plan с координатами и
  страной; exclusions и дедупликация применяются между Places и Details, чтобы
  raw business intent не попадал в adapter и не расходовалась лишняя quota.
- Для `v0.4.0` сформировано исполнимое ТЗ с frozen golden datasets, hard safety
  gates, offline/live evaluation, контролем стоимости и обязательным агрегированным
  release report. Архитектурные документы не меняют runtime-поведение `v0.3.1`;
  реальный Kimi API ещё не подключён и не объявляется проверенным.
- Зафиксированы два SLO-профиля. Для личного Tier-0 пилота fast path без Kimi
  имеет цель `p95 ≤ 15 с`, AI-assisted выдача — `p95 ≤ 55 с`, deadline — `60 с`.
  Production target после минимум Tier-1 — `p95 ≤ 25 с` для AI-assisted выдачи
  и deadline `45 с`. 500 jobs/7 дней дают только калибровку; внешний SLA требует
  минимум 10 000 репрезентативных jobs за 28 дней и статистического подтверждения.
