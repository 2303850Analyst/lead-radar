...
Возвращает health, версию `0.2.0`, активный режим и безопасные признаки
...
Ответ содержит параметры запроса, provider-метаданные, сводку и массив `leads`.
У каждого лида есть массив `sources` с provider ID, внешним ID и временем
наблюдения. Ошибки валидации возвращаются с HTTP 400. Ошибка live-провайдера
...
Geoapify остаётся единственным live-провайдером версии `0.2.0`. Автоматический
