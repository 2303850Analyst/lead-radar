...
Статус: Geoapify подключён как единственный live-провайдер; локальный live smoke
test успешно проверил Geocoding, Places и Place Details. Перед внешним
deployment требуется ротация ключа, ранее появившегося вне server-side env.
...
В `0.2.0` fallback не реализуется. Единственный активный live-источник —
