...
- `DGIS_MAX_PAGES` задаёт глубину выдачи на одну поисковую формулировку.
  Адаптер останавливается раньше, если достигнут заявленный 2GIS `total` или
  получена неполная страница. Demo API ограничивает обход пятью страницами по
  10 карточек независимо от большего значения; production использует до 50
  карточек на страницу, по умолчанию 20 и максимум 100 страниц. Каждая страница
  является отдельным запросом Places API.
...
3. Адаптер выполняет до трёх ограниченных free-text формулировок к
   `https://catalog.api.2gis.com/3.0/items` по основному типу и синонимам. Для
   каждой формулировки он обходит страницы до заявленного `total`, неполной
   страницы или серверного лимита глубины. Каждый запрос получает
   `type=branch`, `q`, `point`/`location`, радиус не более 50 км и в
   demo-режиме `page_size` не более 10.
